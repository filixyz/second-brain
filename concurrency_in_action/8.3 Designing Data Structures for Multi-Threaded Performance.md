...
With this we can now simplify our parallel version of accumulate:
```c++
template<typename Iterator, typename T>
T oarallel_accumulate(Iterator first, Iterator last, T init)
{
	unsigned long const length=std::distance(first, last);
	if(!length)
		return init;
	...
	unsigned long const block_size=length/num_threads;
	std::vector<std::future<T>> futures(num_threads-1);
	std::vector<std::threads> threads(num_threads-1):
...
The previous examples elucidates how to ensure exception safety when managing the threads ourselves, Now we will be doing the same but with `std::async` so we can relinquish the responsibility of managing threads to the c++ threading library.

The key thing to note for exception safety is that if a future is destroys without being waited for, the destructor will wait for the thread to complete, this neatly handles the concern of leaking threads, which are still executing and holding references to data.

This listing shows an exception safe implementation using `std::async`:
```c++
template<typename Iterator, typename T>
T parallel_accumulate(Iterator first, Iterator last, T init)
{
	unsigned long const length=std::advance(first, last);
	...
	else 
	{
		...
		T second half_result=parallel_accumulate(mid_point, last, T());
		return first_half_result.get()+second_half_result;
	}
}
```
This version uses a recursive division of data rather than pre-calculating the division of the data into chunks, and is a lot simpler than it’s previous version and still exception safe.