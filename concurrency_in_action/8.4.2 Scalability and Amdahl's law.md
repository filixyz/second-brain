...
Single threaded code can make even-driven programming hard to write and complex to conceptualize, In single threaded event driven programs, tasks must periodically suspend itself and return control to the event loop to give other tasks the opportunities to execute, This complicates everything.

By separating concerns with concurrency, once can put lengthy tasks on a whole new thread and leave a dedicated GUI thread to process the events, the threads ca then communicate through simple mechanisms:

A simplified example of this concept is illustrated by the following listing:
```c++
...
```
By separating concerns this way, the user thread is always able to respond to events in a timely fashion, even if task takes a long time.